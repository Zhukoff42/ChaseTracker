                ...
                if file then
                    ...
                    if data and data.version then
                        if tostring(data.version) ~= tostring(cur_version) then
                            if tostring(data.version) == tostring(settings.main.skip_version) then
                                update_state = "Обновление v" .. data.version .. " пропущено пользователем."
...
local render_update_window = imgui.OnFrame(function() return update_available end, function(player)
    ...
    imgui.SetNextWindowSize(imgui.ImVec2(380, 230), imgui.Cond.Always)
    
    imgui.PushStyleColor(imgui.Col.WindowBg, imgui.ImVec4(0.1, 0.1, 0.1, 1.0))
    
    -- Передаем nil вместо imgui.new.bool(true), чтобы отключить неработающий крестик
    if imgui.Begin(u8"Доступно обновление ChaseTracker", nil, bit.bor(imgui.WindowFlags.NoCollapse, imgui.WindowFlags.NoResize, imgui.WindowFlags.NoSavedSettings)) then
        ...
        imgui.Spacing()
        if imgui.Button(u8"Напомнить позже (до перезапуска)", imgui.ImVec2(-1, 25)) then
            update_available = false
        end
        if imgui.Button(u8"Пропустить эту версию навсегда", imgui.ImVec2(-1, 25)) then
            settings.main.skip_version = new_version
            saveSettings()
            update_available = false
            update_state = "Обновление v" .. new_version .. " пропущено."
        end
    end
    imgui.End()
    imgui.PopStyleColor()
...
function loadSettings()
    ...
    settings.main.skip_version = settings.main.skip_version or default_cfg.main.skip_version
