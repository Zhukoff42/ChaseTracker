...
local cur_version = thisScript().version or '1.2'
    ...
    if imgui.Begin(u8"Настройки ChaseTracker", ui_menu, imgui.WindowFlags.NoCollapse) then
        if imgui.BeginTabBar("Tabs") then
            ...
            if imgui.BeginTabItem(u8"Внешний вид HUD") then
                imgui.Text(u8"Автоматизация:")
                if imgui.Checkbox(u8"Авто преследование", cb_auto_pursuit) then settings.main.auto_pursuit = cb_auto_pursuit[0] saveSettings() end
