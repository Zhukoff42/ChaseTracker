...
function check_update()
    ...
    downloadUrlToFile(update_url .. "?t=" .. os_time(), update_path, function(id, status, p1, p2)
        if status == dlstatus.STATUSEX_ENDDOWNLOAD then
            ...
            if file then
                ...
                update_state = "Ошибка чтения файла обновления."
            end
...
function perform_update()
    ...
    downloadUrlToFile(script_url .. "?t=" .. os_time(), script_path, function(id, status, p1, p2)
        if status == dlstatus.STATUSEX_ENDDOWNLOAD then
            -- Читаем скачанный файл (он будет в UTF-8 из GitHub)
            ...
            if file then
                ...
                -- Декодируем из UTF-8 в CP1251 для исправления каракулей
                local decoded_content = u8:decode(content)
                
                -- Перезаписываем файл в правильной кодировке
                local file_w = io.open(script_path, "w")
                if file_w then
                    file_w:write(decoded_content)
                    file_w:close()
                end
            end
            ...
            thisScript():reload()
        elseif status == dlstatus.STATUS_ENDDOWNLOADDATA then
            update_state = "Ошибка скачивания обновления."
        end
    end)
end
...
local render_update_window = imgui.OnFrame(function() return update_available end, function(player)
    ...
    imgui.SetNextWindowSize(imgui.ImVec2(350, 170), imgui.Cond.FirstUseEver)
    
    if imgui.Begin(u8"Доступно обновление ChaseTracker!", imgui.new.bool(true), bit.bor(imgui.WindowFlags.NoCollapse, imgui.WindowFlags.NoResize)) then
        imgui.Text(u8("Текущая версия: " .. cur_version))
        imgui.TextColored(imgui.ImVec4(0.2, 1.0, 0.2, 1.0), u8("Новая версия: " .. new_version))
        imgui.Separator()
        
        imgui.Spacing()
        if imgui.Button(u8"ОБНОВИТЬ", imgui.ImVec2(-1, 40)) then
            perform_update()
...
function main()
    ...
    while true do
        ...
        render_update_window.HideCursor = not update_available
