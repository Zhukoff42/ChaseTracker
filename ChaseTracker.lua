...
local update_state = "I?eaaiea i?iaa?ee..."
...
function getKeyName(id)
    if not id or id == 0 then return "IA IACIA?AII" end
    return keys[id] or tostring(id)
end

function check_update()
    update_state = "I?iaa?ea iaiiaeaiee..."
            ...
            if file then
                ...
                if data and data.version then
                    if data.version ~= cur_version then
                        ...
                        update_state = "Ainooiii iaiiaeaiea: v" .. new_version
                    else
                        update_state = "O aan onoaiiaeaia iineaaiyy aa?ney."
                    end
                else
                    update_state = "Ioeaea ia?neiaa Update.json"
                end
            end
        elseif status == dlstatus.STATUS_ENDDOWNLOADDATA then
            update_state = "Ioeaea nea?eaaiey oaeea i?iaa?ee."
        end
    end)
end

function perform_update()
    update_state = "Nea?eaaiea iaiiaeaiey..."
    downloadUrlToFile(script_url, script_path, function(id, status, p1, p2)
        if status == dlstatus.STATUSEX_ENDDOWNLOAD then
            update_state = "Iaiiaeaiea caaa?oaii! Ia?acaa?ocea..."
            sampAddChatMessage("[ChaseTracker] Ne?eio iaiiaeai ai aa?nee " .. new_version .. ". Ia?acaa?ocea...", 0x00FF00)
            thisScript():reload()
        end
    end)
end
...
local function draw_sys_bind(label, bind_key_name)
    ...
    local btn_text = (ui_waiting_bind == bind_key_name) and u8"?AAI IA?AOE? (ESC - NA?IN)" or (current_key == 0 and u8"IA IACIA?AII" or u8(getKeyName(current_key)))
...
local render_menu = imgui.OnFrame(function() return ui_menu[0] end, function(player)
    ...
    if imgui.Begin(u8"Iano?ieee ChaseTracker (Kevin_Zhukoff [Yuma])", ui_menu, imgui.WindowFlags.NoCollapse) then
        if imgui.BeginTabBar("Tabs") then
            
            if imgui.BeginTabItem(u8"Aiaoiee aea HUD") then
                imgui.Text(u8"Yeaiaiou ia ye?aia:")
                if imgui.Checkbox(u8"Iieacuaaou Iee", cb_show_nick) then settings.main.show_nick = cb_show_nick[0] saveSettings() end
                if imgui.Checkbox(u8"Iieacuaaou ID", cb_show_id) then settings.main.show_id = cb_show_id[0] saveSettings() end
                if imgui.Checkbox(u8"Iieacuaaou Noaoon", cb_show_status) then settings.main.show_status = cb_show_status[0] saveSettings() end
                if imgui.Checkbox(u8"Iieacuaaou Oaeia? /z", cb_show_z) then settings.main.show_z_timer = cb_show_z[0] saveSettings() end
                
                imgui.Separator()
                imgui.Text(u8"Oaeno e Oaaoa:")
                if imgui.SliderFloat(u8"?acia? oaenoa (Scale)", slider_scale, 0.5, 3.0) then settings.main.hud_scale = slider_scale[0] saveSettings() end
                if imgui.ColorEdit4(u8"Oaao iniiaiiai oaenoa", color_text) then
                ...
                if imgui.ColorEdit4(u8"Oaao oaeia?a /z", color_z) then
                ...
                imgui.Separator()
                imgui.Text(u8"No?aeea iaa aieiaie:")
                if imgui.Checkbox(u8"Aee??eou ia?ea? iaa oaeu?", cb_show_arrow) then settings.main.show_arrow = cb_show_arrow[0] saveSettings() end
                if imgui.Checkbox(u8"Aieiaoey (ieaaaao aaa?o-aiec)", cb_arrow_anim) then settings.main.arrow_anim = cb_arrow_anim[0] saveSettings() end
                if imgui.SliderFloat(u8"?acia? ia?ea?a", slider_arrow_size, 5.0, 50.0) then settings.main.arrow_size = slider_arrow_size[0] saveSettings() end
                
                if imgui.ColorEdit4(u8"Oaao ia?ea?a", color_arrow) then
                ...
                imgui.Separator()
                if imgui.Button(ui_edit_hud[0] and u8"NIO?AIEOU IICEOE? HUD" or u8"IA?AIANOEOU HUD", imgui.ImVec2(-1, 40)) then
            ...
            if imgui.BeginTabItem(u8"Nenoaiiua Aeiau") then
                imgui.TextColored(imgui.ImVec4(0.5, 0.5, 0.5, 1), u8"Ia?ieoa ia eiiieo, ?oiau iacia?eou eeaaeoo. Ia?ieoa ESC aey na?ina.")
                imgui.Separator()
            ...
            if imgui.BeginTabItem(u8"Naie Eiiaiau") then
                imgui.Text(u8"Aiaaaeou eanoiiiue aeia:")
                imgui.PushItemWidth(300)
                imgui.InputTextWithHint("##cmd", u8"/su {id} 3 Iaiia?eiaiea", ui_inputCmd, 256)
                imgui.PopItemWidth()
                imgui.SameLine()
                
                local keyName = (ui_isWaitingKey) and u8"?AO..." or (ui_currentKey == 0 and u8"AUA?AOU" or u8(getKeyName(ui_currentKey)))
                if imgui.Button(keyName, imgui.ImVec2(100, 25)) then ui_isWaitingKey = true end
                
                if imgui.Button(u8"AIAAAEOU", imgui.ImVec2(-1, 30)) then
                ...
                imgui.BeginChild("BindList", imgui.ImVec2(0, -10), true)
                    for i, bind in ipairs(settings.binds) do
                        ...
                        if imgui.Button(u8"Oaaeeou") then
            ...
            if imgui.BeginTabItem(u8"Eioi?iaoey") then
                imgui.Text(u8"Ne?eio: ChaseTracker")
                imgui.Text(u8"Aaoi?: Zhukoff Kevin_Zhukoff [Yuma]")
                imgui.Text(u8"Oaeouay aa?ney: " .. cur_version)
                imgui.Separator()
                imgui.Text(u8"Noaoon: " .. u8(update_state))
                
                if imgui.Button(u8"I?iaa?eou iaiiaeaiey", imgui.ImVec2(-1, 35)) then
                    check_update()
                end
                
                if update_available then
                    if imgui.Button(u8"Onoaiiaeou iaiiaeaiea", imgui.ImVec2(-1, 35)) then
                        perform_update()
                    end
                end
...
local render_hud = imgui.OnFrame(function() return chase.active or ui_edit_hud[0] end, function(player)
    ...
    if imgui.Begin("ChaseHUD", nil, flags) then
        if ui_edit_hud[0] then
            ...
            imgui.TextColored(imgui.ImVec4(1, 1, 0, 1), u8"?A?EI ?AAAEOE?IAAIE? (Aaeaae ieii)")
        end
        ...
        if settings.main.show_nick then imgui.Text(u8("Oaeu: " .. tostring(chase.name or ""))) end
        if settings.main.show_id then imgui.Text(u8("ID: " .. tostring(chase.id or -1))) end
        if settings.main.show_status then 
            local st = (chase.status == nil or chase.status == "") and "Aeoeaia" or chase.status
            imgui.Text(u8("Noaoon: " .. tostring(st))) 
        end
...
function main()
    ...
    sampAddChatMessage("[ChaseTracker] v" .. cur_version .. " by Kevin_Zhukoff [Yuma] caa?o?ai. Iai?: {FFFF00}/chase", 0x00FF00)
    ...
    sampRegisterChatCommand("trg", function(arg)
        ...
        if #arg == 0 then
            resetChase()
            sampAddChatMessage("[ChaseTracker] ?o?iay oaeu na?ioaia.", 0xFFFF00)
            return
        end
        local id = tonumber(arg:match("%d+"))
        if id and sampIsPlayerConnected(id) then 
            setTarget(id, "?o?iie oa?aao (/trg)")
        else
            sampAddChatMessage("[ChaseTracker] Ea?ie n oaeei ID ia iaeaai!", 0xFF0000)
        end
    end)
    ...
    while true do
        ...
        if chase.active then
            if not sampIsPlayerConnected(chase.id) or sampGetPlayerNickname(chase.id) ~= chase.name then
                resetChase()
                sampAddChatMessage("[ChaseTracker] Oaeu iieeioea na?aa? eee niaieea ID. Ionea?eaaiea caaa?oaii.", 0xFF6347)
            end
        end
            ...
            if rmbTargetWindow > 0 and os.clock() <= rmbTargetWindow then
                ...
                if res and targetPed and doesCharExist(targetPed) then
                    ...
                    if resId and targetId ~= -1 then
                        setTarget(targetId, "Oa?aao (2x IEI)")
                        rmbTargetWindow = 0
                    end
                end
            end
            ...
            if not ui_edit_hud[0] and not ui_menu[0] then
                ...
                if chase.active then
                    ...
                    if (settings.main.bind_cuff or 0) ~= 0 and wasKeyPressed(settings.main.bind_cuff) then 
                        sampSendChat("/me niye ia?o?ieee n oaeoe?aneiai iiyna e ?aceei aae?aieai caeiaae ?oee iiaic?aaaaiiai")
                        sampProcessChatInput("/cuff " .. chase.id) 
                    end
                    
                    if (settings.main.bind_pull or 0) ~= 0 and wasKeyPressed(settings.main.bind_pull) then 
                        sampSendChat("/me neeie ioe?ue aaa?u o?ainii?oa e auoauee iiaic?aaaaiiai ia?o?o")
                        sampProcessChatInput("/pull " .. chase.id) 
                    end
                    
                    if (settings.main.bind_gotome or 0) ~= 0 and wasKeyPressed(settings.main.bind_gotome) then 
                        sampSendChat("/me acye caaa??aiiiai ca caeiieaiio? ?oeo e oaa?aiii iiaae ca niaie")
...
function setTarget(id, statusText)
    if id and sampIsPlayerConnected(id) then
        ...
        chase.status = statusText or "?o?iie oa?aao"
        sampAddChatMessage(string.format("[ChaseTracker] Onoaiiaeaia oaeu: %s [%d]", chase.name, chase.id), 0x00FF00)
    end
end
...
function sampev.onServerMessage(color, text)
    ...
    if cleanText:find("Au oniaoii ia?aee iiaii? ca ea?ieii") or cleanText:find("ia?aee ionea?eaaou") or cleanText:find("ia?aee i?aneaaiaaiea") then
        local name, id = cleanText:match("([A-Za-z0-9_]+)%s*%[ID:%s*(%d+)%]")
        if not name or not id then
            name, id = cleanText:match("ea?ieii%s+([A-Za-z0-9_]+).*ID:%s*(%d+)")
        end
        if name and id then 
            chase.active = true; chase.name = name; chase.id = tonumber(id); chase.status = "Iiaiiy (/pursuit)" 
        end
    end
    if cleanText:find("Au oniaoii iiiaoeee ea?iea") and cleanText:find("oa?aiea") then
        local name = cleanText:match("iiiaoeee ea?iea%s+(.+)%.%s+Anee")
        local minutes = cleanText:match("oa?aiea%s+(%d+)%s+ieioo")
        if name and minutes then chase.z_expiry = os.time() + (tonumber(minutes) * 60); chase.z_name = name end
    end
    if cleanText:find("I?aneaaiaaiea.*i?einoaiiaeaii") or cleanText:find("ioi?aaee%(a%) iiaic?aaaaiiai.*a EIC") or cleanText:find("iioa?yee ec aeao") or cleanText:find("iiaiiy auea i?ae?auaia") then 
        resetChase() 
    end
end
